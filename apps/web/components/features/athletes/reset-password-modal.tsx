...
interface Props {
  athlete: { id: string; full_name: string; username: string | null };
  onSuccess: () => void;
}
...
export function ResetPasswordModal({ athlete, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError(null);
    try {
      const res = await fetch("/api/athletes/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ athlete_id: athlete.id, new_password: password }),
      });
          ...
          {view === "form" ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-sm text-muted-foreground">
                ...
                {athlete.username ? ` (@${athlete.username})` : ""} için yeni bir şifre önerildi —
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                <strong>{athlete.full_name}</strong> artık bu yeni şifreyle giriş yapabilir.
              </p>
